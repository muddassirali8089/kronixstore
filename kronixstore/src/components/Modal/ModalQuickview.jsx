...
const ModalQuickview = () => {
    ...
    const percentSale = selectedProduct && Math.floor(100 - ((selectedProduct.price / selectedProduct.mrp) * 100))
    ...
    const handleIncreaseQuantity = () => {
        if (selectedProduct) {
            selectedProduct.min_qty += 1
            updateCart(selectedProduct.id, selectedProduct.min_qty + 1, activeSize, activeColor);
        }
    };

    const handleDecreaseQuantity = () => {
        if (selectedProduct && selectedProduct.min_qty > 1) {
            selectedProduct.min_qty -= 1
            updateCart(selectedProduct.id, selectedProduct.min_qty - 1, activeSize, activeColor);
        }
    };
    ...
    const handleAddToCart = () => {
        if (selectedProduct) {
            if (!cartState.cartArray.find(item => item.id === selectedProduct.id)) {
                ...
                updateCart(selectedProduct.id, selectedProduct.min_qty, activeSize, activeColor)
            } else {
                updateCart(selectedProduct.id, selectedProduct.min_qty, activeSize, activeColor)
            }
            openModalCart()
                        ...
                        <div className="right w-full px-4">
                            ...
                            <div className="product-infor px-4">
                                ...
                                <div className="flex items-center gap-3 flex-wrap mt-5 pb-6 border-b border-line">
                                    ...
                                    <div className="product-origin-price font-normal text-secondary2"><del>${selectedProduct?.mrp}.00</del></div>
                                    {selectedProduct?.mrp && (
                                ...
                                <div className="list-action mt-6">
                                    ...
                                    <div className="choose-qty flex items-center max-xl:flex-wrap lg:justify-between gap-5 mt-3">
                                        <div className="qty-block md:p-3 max-md:py-1.5 max-md:px-3 flex items-center justify-between rounded-lg border border-line sm:w-[180px] w-[120px] flex-shrink-0">
                                            <Icon.Minus
                                                onClick={handleDecreaseQuantity}
                                                className={`${selectedProduct?.min_qty === 1 ? 'disabled' : ''} cursor-pointer body1`}
                                            />
                                            <div className="body1 font-semibold">{selectedProduct?.min_qty}</div>
                                            <Icon.Plus
