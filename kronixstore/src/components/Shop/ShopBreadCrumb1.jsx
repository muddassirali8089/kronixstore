...
const ShopBreadCrumb1 = ({ data, productPerPage, dataType, gender, category, selectCategory }) => {
   console.log("🛍️ ShopBreadCrumb1 received data:", data);
    ...
    let filteredData = data.filter(product => {
        ...
        if (selectedCategory) {
            isDataCategoryMatched = product.category === selectedCategory;
        }
        ...
        if (size) {
            ...
            const output = product.sizes.map(str => str.toUpperCase());
            isSizeMatched = output.includes(size)
        }
    ...
    if (sortOption === 'discountHighToLow') {
        filteredData = sortedData
            .sort((a, b) => (
                (Math.floor(100 - ((b.price / b.mrp) * 100))) - (Math.floor(100 - ((a.price / a.mrp) * 100)))
            ))
    }
                    ...
                    <div className="container lg:pt-[134px] pt-24 pb-10 relative">
                        <div className="main-content w-full h-full flex flex-col items-center justify-center relative z-[1]">
                            <div className="text-content">
                                <div className="heading2 text-center">{selectCategory === null ? 'Shop' : selectCategory}</div>
                                <div className="link flex items-center justify-center gap-1 caption1 mt-3">
                                    <Link href={'/'}>Home</Link>
                                    <Icon.CaretRight size={14} className='text-secondary2' />
                                    <div className='text-secondary2 capitalize'>{selectCategory === null ? 'Shop' : selectCategory}</div>
                                </div>
                            </div>
                        </div>
                    </div>
