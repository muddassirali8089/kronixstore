...
const Product = ({ data, type, style }) => {
    ...
    const handleAddToCart = () => {
        if (!cartState.cartArray.find(item => item.id === data.id)) {
            addToCart({ ...data });
            updateCart(data.id, data.min_qty, activeSize, activeColor)
        } else {
            updateCart(data.id, data.min_qty, activeSize, activeColor)
        }
        openModalCart()
    ...
    let percentSale = Math.floor(100 - ((data.price / data.mrp) * 100))
    // { console.log(data) }
    let percentSold = Math.floor((data.sold / data.qty) * 100)
                    ...
                    <div onClick={() => handleDetailProduct(data.id)} className="product-main cursor-pointer block">
                        <div className="product-thumb bg-white relative overflow-hidden rounded-2xl">
                            ...
                            <div className="product-img w-full h-full aspect-[3/4]">
                                {activeColor ? (
                                    <Image
                                        src={data.variations.find(v => v.color === activeColor)?.image || data.images}
                                        width={500}
                                        height={500}
                                        alt={data.name}
                                        priority={true}
                                    ...
                                    <Image
                                        src={`http://localhost:3030${data.images[0] || data.thumbImage?.[0] || '/images/default.png'}`}
                                        width={500}
                                        height={500}
                                        alt={data.name}
                                        priority={true}
                        ...
                        <div className="product-infor mt-4 lg:mb-7">
                            <div className="product-sold sm:pb-4 pb-2">
                                ...
                                <div className="flex items-center justify-between gap-3 gap-y-1 flex-wrap mt-2">
                                    ...
                                    <div className="text-button-uppercase">
                                        ...
                                        <span className='max-sm:text-xs'>{data.qty - data.sold}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="product-price-block flex items-center gap-2 flex-wrap mt-1 duration-300 relative z-[1]">
                                ...
                                {percentSale > 0 && (
                                    <>
                                        <div className="product-origin-price caption1 text-secondary2"><del>₹{data.mrp}</del></div>
