...
const Default = ({ data, productId }) => {
    ...
    const handleIncreaseQuantity = () => {
        productMain.min_qty += 1
        updateCart(productMain?.id, productMain?.min_qty + 1, activeSize, activeColor);
    };

    const handleDecreaseQuantity = () => {
        if (productMain?.min_qty > 1) {
            productMain.min_qty -= 1
            updateCart(productMain?.id, productMain?.min_qty - 1, activeSize, activeColor);
        }
    };
    ...
    const handleAddToCart = () => {
        if (!cartState.cartArray.find(item => item.id === productMain?.id)) {
            ...
            updateCart(productMain?.id, productMain?.min_qty, activeSize, activeColor)
        } else {
            updateCart(productMain?.id, productMain?.min_qty, activeSize, activeColor)
        }
        openModalCart()
    ...
    console.log(productMain?.variations)
                    ...
                    <div className="container flex justify-between gap-y-6 flex-wrap">
                            ...
                            <div className={`popup-img ${openPopupImg ? 'open' : ''}`}>
                                ...
                                >
                                    {productMain?.images?.map((item, index) => (
                                        ...
                                        >
                                            <Image
                                                src={`http://localhost:3030/api${item}`}
                                                width={1000}
                                                height={1000}
                                                alt='prd-img'
                                                className='w-full aspect-[3/4] object-cover rounded-xl'
                        ...
                        <div className="product-infor md:w-1/2 w-full lg:pl-[15px] md:pl-2">
                            ...
                            <div className="flex items-center gap-3 flex-wrap mt-5 pb-6 border-b border-line">
                                ...
                                <div className="product-origin-price font-normal text-secondary2"><del>₹{productMain?.mrp}</del></div>
                                {productMain?.mrp && (
                            ...
                            <div className="list-action mt-6">
                                <div className="choose-color">
                                    ...
                                    <div className="list-color flex items-center gap-2 flex-wrap mt-3">
                                        {productMain?.variations.map((item, index) => (
                                            <div
                                ...
                                <div className="choose-qty flex items-center lg:justify-between gap-5 gap-y-3 mt-3">
                                    <div className="qty-block md:p-3 max-md:py-1.5 max-md:px-3 flex items-center justify-between rounded-lg border border-line sm:w-[180px] w-[120px] flex-shrink-0">
                                        <Icon.Minus
                                            ...
                                            className={`${productMain?.min_qty === 1 ? 'disabled' : ''} cursor-pointer`}
                                        />
                                        <div className="body1 font-semibold">{productMain?.min_qty}</div>
                                        <Icon.Plus
                                            size={20}
