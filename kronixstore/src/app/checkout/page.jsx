...
const Checkout = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [itemsResponse, categoriesResponse] = axios.get('http://localhost:3030/api/items');
                            ...
                            {val !== 'loggined' && (
                                <>
                                    <div className="login bg-surface py-3 px-4 flex justify-between rounded-lg">
                                        <div className="left flex items-center"><span className="text-on-surface-variant1 pr-4">Don&apos;t have an account? </span><span className="text-button text-on-surface hover-underline cursor-pointer">Register</span></div>
